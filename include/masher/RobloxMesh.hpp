 ...
 * v4.00 is a full overhaul adding support for skeletal data and mesh
 * deformation v5.00 is a slight revision to V4 with additional facial animation
 * data at the end
 */
...
enum RobloxMeshOptimizer {
  OPTIMIZER_NONE = 0,
  OPTIMZIER_UNKNOWN, // Unused, but masher uses this if RobloxMesh->optimizer
                     // was never set
  OPTIMIZER_ROBLOX,
  OPTIMIZER_ZEUX
};
...
public:
  ...
  bool holdsTexWData() {
    return version > ROBLOX_MESH_V1_01 ? false : isTexWDataPresent;
  }
  bool holdsRgbaData() {
    return version > ROBLOX_MESH_V2_00 ? true : isRgbaDataPresent;
  }
...
struct RobloxMeshVertex {
  ...
  float tu, tv, tw; // UV Texture Coordinates (tex_W is only for data retention
                    // with corrupt v1.xx meshes)
...
struct RobloxMeshBone {
  ...
  // CFrame.Angles(rx, ry, rz)) The position of the face bone is a translation
  // offset relative to the rotation (i.e. rotation * CFrame.new(position))
